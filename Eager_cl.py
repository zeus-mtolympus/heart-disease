...
class Eager:
    ...
    def fit(self, Xsel, y, original_feature_names):
        ...
        # Fix #4: label_support computed after minimality filtering, not before
        candidate_rules = []
        ...
        # Fix #4: build label_support only from final minimal rules, not all candidates
        label_support = {}
        for r in final_rules:
            label_support[r["label"]] = label_support.get(r["label"], 0) + r["repet"]
    ...
    def print_rules(self, top_n=20):
        # Fix #5: guard against empty rules
        if not self.rules:
