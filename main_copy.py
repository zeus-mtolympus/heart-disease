...
lazy_lad = LazyLAD(
    binarizer=binarizer,
    selector=selector,
    purity=0.70,       # often best in lazy mode
    min_support=4,
    verbose=True
).fit(Xsel_train, y_train)

# BOTH work perfectly:
results_lazy = evaluate(
    mp=lazy_lad,
    X_train_sel=Xsel_train,
    y_train=y_train,
    X_test_sel=Xsel_test,
    y_test=y_test,
    model_name="Lazy LAD (Final)"
)

# And this too:
preds, scores, reasons = evaluate_rules_strongest_partial(lazy_lad, Xsel_test, y_test)